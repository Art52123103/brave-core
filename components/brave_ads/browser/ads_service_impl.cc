...
std::vector<ads::AdConversionInfo> GetAdConversionsOnFileTaskRunner(
    const std::string& url,
    BundleStateDatabase* backend) {
  std::vector<ads::AdConversionInfo> ad_conversions;
...
void AdsServiceImpl::SetOptOutOfAdConversions(
    const bool should_opt_out) {
  SetBooleanPref(prefs::kShouldOptOutOfAdConversions, should_opt_out);
}
...
bool AdsServiceImpl::ShouldOptOutOfAdConversions() const {
  return GetBooleanPref(prefs::kShouldOptOutOfAdConversions);
}
...
void AdsServiceImpl::OnGetAdConversions(
    ...
    const std::string& url,
    const std::vector<ads::AdConversionInfo>& ad_conversions) {
  if (!connected()) {
    return;
  }
