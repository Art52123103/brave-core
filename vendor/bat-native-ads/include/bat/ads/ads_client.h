...
using OnGetAdConversionsCallback = std::function<void(const Result,
    const std::string&, const std::vector<AdConversionInfo>&)>;
...
class ADS_EXPORT AdsClient {
 public:
  ...
  // Should return |true| if opt out of ad conversions is enabled; otherwise,
  // should return |false|
  virtual bool ShouldOptOutOfAdConversions() const = 0;
